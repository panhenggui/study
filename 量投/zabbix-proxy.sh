...
init()
{
        ...
        yum install -y gcc curl-devel net-snmp-devel mariadb mariadb-server mariadb-devel wget java-1.7.0-openjdk-devel java-1.7.0-openjdk
        ...
        mysql -e "create database zabbix character set utf8 collate utf8_bin;grant all on zabbix.* to zabbix@localhost identified by '3PVv3b7S'"
...
install()
{
        ...
        cd zabbix-3.2.1
        ./configure --enable-agent --enable-java --enable-proxy --with-libcurl --with-mysql --with-net-snmp --prefix=${prefix}
        make -j
        make install
        #import db
