myapp.controller('OperOrderServiceController', function ($scope, $timeout) {
	 $scope.queryEntity = {
	    		accountID:"",
	    		instrumentID:"",
	    		brokerageFirmID:"",
    ...
    $scope.find = function (queryEntity)  {
    	...
    	if($scope.tmpQuery.searchType=="1"){
    		...
    		if($scope.tmpQuery.accountID == undefined || $scope.tmpQuery.accountID == ""){
    			findOperOrderAllCapitalByQuery(function(result){
	    			$scope.sumVolumeTraded = 0;//申请量
	    		    $scope.sumVolume = 0;//申请量
	    		    $scope.count=0;
...
//    	    		}
	    			$scope.$apply();
	    		},{instrumentID:$scope.tmpQuery.instrumentID,orderStatus:$scope.tmpQuery.orderStatus,investorType:"1"});
    		}else{
    			findOperOrderAllCapitalByQuery(function(result){
	    			$scope.sumVolumeTraded = 0;//申请量
	    		    $scope.sumVolume = 0;//申请量
...
//    	    		}
    		...
    		if($scope.tmpQuery.accountID == undefined || $scope.tmpQuery.accountID == ""){
    			findOperOrderAllTraderByQuery(function(result){
	    			$scope.sumVolumeTraded = 0;//申请量
	    		    $scope.sumVolume = 0;//申请量
	    		    $scope.count=0;//一共多少笔
	    			$scope.listEntitys =result;
	    			...
	    			$scope.$apply();
	    		},{instrumentID:$scope.tmpQuery.instrumentID,orderStatus:$scope.tmpQuery.orderStatus,investorType:"2"});
    		}else{
    			findByQueryOperOrderAccount(function(result){
	    			$scope.sumVolumeTraded = 0;//申请量
	    		    $scope.sumVolume = 0;//申请量
	    		    $scope.count=0;//一共多少笔
	    			$scope.listEntitys =result;
    ...
    $scope.orderFailedQuery = function ( opts ) {
        return function ( request, drawCallback, settings ) {        	
            ...
            if(orderT == "asc"){
                ...
                var tempArray =[];
                findAllOperOrderQueryInMapper($scope.tmpQuery, requestParams, $scope.orderType, $scope.orderWay, function(result){
...
function initPage(){
	...
	var table = $('#operOrder_table').DataTable( {
	 	columns :[ 
					{ title: "报单编号" },
					{ title: "申报时间" },
					{ title: "账号" },
		 ...
		 "footerCallback": function () {
			 $scope.api = this.api();
			  $timeout(function() {
			    	 ...
			    	 $($(".dataTables_scrollFoot .operOrderFoot tr")[0].childNodes[9]).html(
			    			 $scope.api.column( 9, {page:'current'} ).data().sum());
			    	 $($(".dataTables_scrollFoot .operOrderFoot tr")[0].childNodes[10]).html(
...
$scope.find = function(queryEntity){
	...
	//导出excel
	$scope.exportExcelOO = function (queryEntity) {
		$scope.tmpQuery=angular.copy(queryEntity);
		if($scope.tmpQuery.accountID != null){
	   	var accountEntity = angular.copy($scope.tmpQuery.accountID);
	       delete $scope.tmpQuery.accountID;
		...
		framework.file.export("实时委托查询.xls",'excel',{
			entityKey:['orderSysID','insertTime','accountID','accountName','instClientName',
			           ...
			           'eachMarginStr','tradeUser','investManager','userID','subOrderSysID',
		...
		},"operOrderService","exportExcelOO",$scope.tmpQuery);
	};
