myapp.controller('RealTimeAccountQueryController', function ($scope, $rootScope,$timeout) {
	//调用表格重绘函数
    ...
    // 按钮权限
    $scope.realTimeAccountQuery_query = isShow("realTimeAccountQuery_query");
    $scope.realTimeAccountQuery_export = isShow("realTimeAccountQuery_export");
...
/*	
	...
	$scope.changeInstClient = function(){
		if("" == $scope.queryEntity.brokerageFirmID){
			$scope.queryEntity.brokerageFirmID = null;
		}
		//资金账号
		if($scope.queryEntity.searchType == 1){
			$scope.typeName = "资金账号";
    ...
    function pushEntitys(s, type){
		$scope.tEntitys = [];
		$scope.tEntitys = new Array();
		if(type == 1){
			angular.forEach(s, function (value, index, arrays) {
				$scope.tmpEntitys = {"innerAccountID":"", "accountName":"", "instClientID":""};
				$scope.tmpEntitys.innerAccountID = value.innerAccountID;
	...
	$scope.find = function(accountID){
		...
		$scope.operInvestorAccountService.findBySubAccountIdAndAccountId(function(result){
    ...
    $scope.operClientPositionQuery = function ( opts ) {
        return function ( request, drawCallback, settings ) {        	
            ...
            if(orderT == "asc"){
                ...
                var tempArray =[];
                findAllRealAccountByQueryInMapper($scope.tmpQuery, requestParams, $scope.orderType, $scope.orderWay, function(result){
                    ...
                    /*if(con != null && con.length > 1){
                    	for(var i = 0; i < con.length - 1;i++){
                        	...
                        	var tempArr = [con[i].traderID,con[i].accountName,con[i].instClientName,
                                           ...
                                           parseFloat(con[i].dynamicRights).toFixed(2),parseFloat(con[i].premiumIn - con[i].premiumOut ).toFixed(2),(con[i].risk *100).toFixed(2)+"%",$scope.transCurrencys(con[i].currency)];
   
                            tempArray.push(tempArr);
                        }
                            $scope.summaryInfo = {};
                            $scope.summaryInfo = con[con.length - 1];
                    }else if(con != null && con.length <= 1){
                    	$scope.summaryInfo = {};
                        $scope.summaryInfo = con[0];
                    }*/
                    
                    for(var i = 0; i < con.length;i++){

                    	var tempArr = [con[i].traderID,con[i].accountName,con[i].instClientName,
                    	               parseFloat(con[i].preBalance).toFixed(2),parseFloat(con[i].deposit-con[i].withdraw).toFixed(2),  parseFloat(con[i].available).toFixed(2), parseFloat(con[i].margin).toFixed(2),
                    	               parseFloat(con[i].fee).toFixed(2), parseFloat(con[i].frozenMargin).toFixed(2),parseFloat(con[i].frozenFee).toFixed(2),
                    	               parseFloat(con[i].closeProfit).toFixed(2), parseFloat(con[i].positionProfit).toFixed(2),
                                       parseFloat(con[i].dynamicRights).toFixed(2),parseFloat(con[i].premium).toFixed(2),(con[i].risk *100).toFixed(2)+"%",$scope.transCurrencys(con[i].currency)];
...
function initPage(){
	//会话列表初始化
	var table = $('#realAccount_table').DataTable( {
 	columns :[   
				{ title: "账号" },
				{ title: "账号名称" },
				{ title: "所属机构" },
...
/*		      {
	 ...
	 /*"footerCallback": function () {
		 $scope.apii = this.api();
		  $timeout(function() {
			  $($(".dataTables_scrollFoot .realAccountFoot tr")[0].childNodes[6]).html($scope.summaryInfo.margin.toFixed(2));
		      $($(".dataTables_scrollFoot .realAccountFoot tr")[0].childNodes[10]).html($scope.summaryInfo.closeProfit.toFixed(2)); 
		      $($(".dataTables_scrollFoot .realAccountFoot tr")[0].childNodes[11]).html($scope.summaryInfo.positionProfit.toFixed(2)); 
		      $($(".dataTables_scrollFoot .realAccountFoot tr")[0].childNodes[12]).html($scope.summaryInfo.dynamicRights.toFixed(2)); 
		   },"200")
		}*/
} );
}
...
//导出excel
$scope.exportExcelRTA = function (queryEntity) {
	$scope.tmpQuery=angular.copy(queryEntity);
	if($scope.tmpQuery.accountID != null){
    	var accountEntity = angular.copy($scope.tmpQuery.accountID);
        delete $scope.tmpQuery.accountID;
...
//        $scope.tmpQuery.brokerageFirmID = accountEntity.instClientID;
	...
	framework.file.export("实时资金查询.xls",'excel',{
		entityKey:['traderID','accountName','instClientName','preBalanceStr','depositStr','availableStr','marginStr',
		           'feeStr','frozenMarginStr','frozenFeeStr','closeProfitStr','positionProfitStr','dynamicRightsStr','premiumStr','riskStr','currency'],
		headerKey:['账号','账号名称','所属机构','上日结存','今日出入金','可用资金','保证金','手续费','冻结保证金','冻结手续费','平仓盈亏', '持仓盈亏','动态权益','权利金收支','风险度','币种'],
	...
	},"operInvestorAccountService","exportExcelRTA",$scope.tmpQuery);
};
