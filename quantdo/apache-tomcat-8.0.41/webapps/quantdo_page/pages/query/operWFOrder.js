myapp.controller('OperWFOrderServiceController', function ($scope, $rootScope,$timeout) {
	//调用表格重绘函数
    ...
    // 按钮权限
    $scope.operWFOrder_query = isShow("operWFOrder_query");
    $scope.operWFOrder_export = isShow("operWFOrder_export");
    ...
    function initPage(){
        //会话列表初始化
        var table = $('#operWFOrder_dynamic_tableOne').DataTable( {
            columns :[
                {title: "序号"},
                {title: "报单编号"},
                {title: "申报时间"},
            ...
            "footerCallback": function () {
                ...
                $scope.operOrderService.totleOperOrder($scope.tmpQuery, function(retrunData){
                    if(retrunData != null && retrunData != undefined){
                        $( api.column( 11 ).footer() ).html(parseFloat(retrunData.volumeStr).toFixed(0));
                        $( api.column( 12 ).footer() ).html(parseFloat(retrunData.volumeTradedStr).toFixed(2));
                    }else{
        ...
        //会话列表初始化
        var comtable = $('#operWFOrder_dynamic_tableTwo').DataTable( {
            columns :[
                {title: "序号"},
                {title: "报单编号"},
                {title: "申报时间"},
            ...
            "footerCallback": function () {
                ...
                $scope.operOrderService.totleOperOrder($scope.tmpQuery, function(retrunData){
                    if(retrunData != null && retrunData != undefined){
                        ...
                        $( api.column( 11 ).footer() ).html(parseFloat(retrunData.volumeStr).toFixed(2));
                        $( api.column( 12 ).footer() ).html(parseFloat(retrunData.volumeTradedStr).toFixed(2));
    ...
    $scope.find = function(queryEntity) {
         ...
         return function (request, drawCallback, settings){
			...
			//更新表格对应的数据集
             ...
             if ($scope.isShow == 1) {
                 $scope.operOrderService.findOperWFOrderByCapital($scope.tmpQuery, requestParams, $scope.orderType, $scope.orderWay, function (result) {
                 ...
                 for (var i = 0; i < con.length; i++) {
					...
					var tempArr = [(i + 1), con[i].orderSysID, con[i].insertTime, con[i].investorID, con[i].accountName,
						con[i].instClientName, $scope.formatStr(con[i].branchID, con[i].branchName), con[i].instrumentID,
						$scope.transDirection(con[i].direction), $scope.transOffsetFlag(con[i].offsetFlag),
						parseFloat(con[i].limitPrice).toFixed(3), con[i].volume, con[i].volumeTraded, $scope.transOrderStatus(con[i].orderStatus),
						con[i].exchID, $scope.transTradeType(con[i].hedgeFlag), parseFloat(con[i].eachFee * con[i].volumeRemain).toFixed(2),
						parseFloat(con[i].eachMargin * con[i].volumeRemain).toFixed(2),
						con[i].subOrderSysID, con[i].cancelUserID, con[i].cancelTime, con[i].seatID,
						con[i].userTrader, con[i].userName, con[i].ipAddress, con[i].macAddress,
                 ...
                 $scope.operOrderService.findOperWFOrderByCapital($scope.tmpQuery, requestParams, $scope.orderType, $scope.orderWay, function (result) {
                     ...
                     for (var i = 0; i < con.length; i++) {
                         ...
                         var tempArr = [(i + 1), con[i].orderSysID, con[i].insertTime, con[i].investorID, con[i].accountName,
                             con[i].inneraccountID, con[i].inneraccountName, con[i].instClientName, $scope.formatStr(con[i].branchID, con[i].branchName), con[i].instrumentID,
                             $scope.transDirection(con[i].direction), $scope.transOffsetFlag(con[i].offsetFlag),
                             parseFloat(con[i].limitPrice).toFixed(3), con[i].volume, con[i].volumeTraded, $scope.transOrderStatus(con[i].orderStatus),
                             con[i].exchID, $scope.transTradeType(con[i].hedgeFlag), parseFloat(con[i].eachFee * con[i].volumeRemain).toFixed(2),
                             parseFloat(con[i].eachMargin * con[i].volumeRemain).toFixed(2), con[i].cancelUserID, con[i].cancelTime,
                             con[i].userTrader, con[i].userName, con[i].ipAddress, con[i].macAddress,
                             con[i].investorAdvisor, con[i].investorAdvisorName, con[i].advIpAddr, con[i].advMacAddr,
                             con[i].investorManager, con[i].investorManagerName, con[i].mngIpAddr, con[i].mngMacAddr, con[i].errorMsg
                         ];
...
//            $scope.tmpQuery.instClientID = accountEntity.instClientID;
        ...
        if(queryEntity.searchType==1){
            ...
            var table = $('#operWFOrder_dynamic_tableTwo').DataTable();
            $scope.isCom=true;
            table.draw();
        }
        $timeout(function() {
    ...
    //导出excel
	$scope.exportExcelOWFO = function (queryEntity) {
	 	$scope.tmpQuery = angular.copy(queryEntity);
        if($scope.tmpQuery.accountID != null){
        	var accountEntity = angular.copy($scope.tmpQuery.accountID);
            delete $scope.tmpQuery.accountID;
...
//            $scope.tmpQuery.brokerageFirmID = accountEntity.instClientID;
        ...
        if($scope.tmpQuery.searchType == "1"){	//资金账号
			framework.file.export("实时委托查询.xls",'excel',{
				entityKey:['orderSysID','insertTime','accountID','accountName','instClientName','branchID',
				           ...
				           'cancelUserID','cancelTime','seatID','userTrader','userName','ipAddress','macAddress', 'investorAdvisor', 'investorAdvisorName','advIpAddr','advMacAddr','investorManager','investorManagerName','mngIpAddr','mngMacAddr','errorMsg'],
				 
				           
				headerKey:['报单编号','申报时间','资金账号','资金账号名称','所属机构','营业部',
				           '合约代码','买卖','开平','委托价格','申报量',
				           '成交量','报单状态', '交易所代码','交易类型','冻结手续费',
				           '冻结保证金','子报单编号',
				           '撤单用户','撤单时间', '下单席位号','交易员', '交易员名称', '交易员IP地址','交易员MAC地址','投资助理','投资助理名称','投顾IP地址','投顾MAC地址','投资经理','投资经理名称','投资经理IP地址','投资经理MAC地址','备注'],
			...
			},"operWFOrderService","exportExcelOWFOCap",$scope.tmpQuery);
        } else {
        	framework.file.export("实时委托查询.xls",'excel',{
				entityKey:['orderSysID','insertTime','accountID','accountName','inneraccountID','inneraccountName','instClientName','branchID',
				           'instrumentID','direction','offsetFlag','limitPriceStr','volumeStr',
				           'volumeTradedStr','orderStatus','exchID','hedgeFlag','eachFeeStr',
				           'eachMarginStr',
				           'cancelUserID','cancelTime','userTrader','userName','ipAddress','macAddress','investorAdvisor','investorAdvisorName', 'advIpAddr','advMacAddr','investorManager','investorManagerName','mngIpAddr','mngMacAddr', 'errorMsg'],
				 
				           
				headerKey:['报单编号','申报时间','资产单元','资产单元名称','资金账号','资金账号名称','所属机构','营业部',
				           '合约代码','买卖','开平','委托价格','申报量',
				           '成交量','报单状态', '交易所代码','交易类型','冻结手续费',
				           '冻结保证金',
				           '撤单用户','撤单时间','交易员','交易员名称', '交易员IP地址','交易员MAC地址','投资助理','投资助理名称','投顾IP地址','投顾MAC地址','投资经理','投资经理名称','投资经理IP地址','投资经理MAC地址','备注'],
	           styles:['plainText','plainText','plainText','plainText','plainText','plainText','plainText',
	            ...
	            ,offsetFlag:{'0':"开仓",'1':"平仓",'2':"强平",'3':"平今",'4':"平昨"}
				,orderStatus:{'0':"全部成交",'1':"部分成交还在队列中",'2':"部分成交不在队列中",'3':"未成交还在队列中",'4':"未成交不在队列中",'5':"撤单",'6':"订单已报入交易所未应答",'7':"部分撤单"}}
			},"operWFOrderService","exportExcelOWFOSubCap",$scope.tmpQuery);
        }
	};
});
