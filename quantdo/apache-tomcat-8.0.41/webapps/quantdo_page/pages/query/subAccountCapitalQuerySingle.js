myapp.controller('SubAccountCapitaQuerySingleController', function ($scope, $timeout,$rootScope) {
	...
	$scope.SubAccountCapitalQuerySingleDataset = [];
	
    $scope.subAccountCapitalQuerySingle_query = isShow("subAccountCapitalQuerySingle_query");
    $scope.subAccountCapitalQuerySingle_export = isShow("subAccountCapitalQuerySingle_export");

	
	//定义固定列头
    $scope.SubAccountCapitalQuerySingle_columns = [
        {title: "序号"},
        {title: "结算日"},
		{title: "资产单元"},
		{title: "所属机构"},
...
//            $scope.tmpQuery.instClientID = subAccountEntity.instClientID;
    ...
    $scope.find = function (queryEntity) {  	
        ...
        findBySubAccountQuerySingle(function(result){		
        	$scope.lastInterest = 0;
        	$scope.finalInterest = 0;
        	$scope.balance = 0;
        	$scope.moneyIn = 0;
			...
			for(var i = 0; i<con.length;i++){
		    	...
		    	$scope.SubAccountCapitalQuerySingleDataset.push(tempArr); 
	        ...
	        $scope.SubAccountCapitalTable.rows.add($scope.SubAccountCapitalQuerySingleDataset).draw();
			$scope.$apply();
			$timeout(function() {
                $scope.isQuery = false;
            }, 1000);
...
//		    $scope.tmpExportQuery.instClientID = subAccountEntity.instClientID;
    	...
    	framework.file.export("资产单元资金查询.xls",'excel',{
		...
		},"subAccountCapitalService","findBySubAccountQueryExport", $scope.tmpExportQuery.settleDate, $scope.tmpExportQuery.endDate, $scope.tmpExportQuery.traderID,
    ...
    $(document).ready(function() {
		    	//会话列表初始化
		    	$scope.SubAccountCapitalTable = $('#SubAccountCapitalQuerySingle_dynamic_table').DataTable( {
			    		data : $scope.SubAccountCapitalQuerySingleDataset,
			        	columns :$scope.SubAccountCapitalQuerySingle_columns,
			            dom: 'rt<"bottom"iplB>',
						buttons: [],
