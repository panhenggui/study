 ...
 */
myapp.controller('marketProportionSetController',function($scope,$rootScope) {
	
	 //初始化接口
   $scope.makertservice = new com.quantdo.orgClear.service.PositionMarketProportionSetService();
   //初始化合约类型
   $scope.contractTypes = clearConstant.contractType;
  
 //设置按钮的名称
	var constant_temp = new com.quantdo.orgClear.constant.ClearConstants();
	$scope.marketProportionSet_buttonName_detail_save = constant_temp.buttonName.setPara;
	$scope.marketProportionSet_buttonName_detail_update = constant_temp.buttonName.resetPara;
	...
	$scope.marketProportionSetShow = false;
	$scope.marketProportionSetEdit = true;
    $scope.paramType = "";
   
    //新增
    $scope.showDetail = function(){
    	$scope.marketProportionSetShow = true;
    	$scope.marketProportionSetEdit = true;	
    	$scope.addPostionSet();
    }
    //取消修改
   ...
   //数据初始化下拉框信息
   $scope.makertservice.queryRiskGropAndDic($scope.pageProper,sessionStorage.listBrokerID,function(result){
   	if(result){
...
// 保存信息实体
   ...
   $scope.initSelect = function(){
   	...
   	$scope.denominator = [];
   	$scope.riskgroup = [];
   	$("input[name=marketProportionSet_contractType]:checked").each(function(){
...
//       	   tempArr.push(parseFloat($scope.entity.threshold).toFixed(2) + "%");
   ...
   $scope.dataset = [];
   //数据初始化
   $scope.markerProportionSetAddButtonIsShow = false;
   $scope.tplID = '8';
   $scope.initTableData = function(isControl){
	   $scope.makertservice.findRiskFundProductByProductID($scope.tplID,sessionStorage.listFundID,sessionStorage.listBrokerID,function(result){
   		if(result){
   			$scope.dataset = [];
//   			destroyDatatable("marketProportionSetSet_table");
               var con = result;
...
//             	   tempArr.push(parseFloat(con[i].threshold).toFixed(2) + "%");
               	var tempArr = [i+1,tmpmarket,con[i].groupName,con[i].particlesizeName,
               	               ...
               	               "<a style='cursor:pointer;' name='PositionMarketProportionSetService.update' class='market_updateIndex backFilter'>[修改]</a> " +
               	               "<a style='cursor:pointer;' name='PositionMarketProportionSetService.delete' class='market_deleteIndex backFilter'>[删除]</a> ",
   	   ...
   	   $scope.markerProportionSetAddButtonIsShow = true;
	   $scope.$apply();
   	});
   ...
   $scope.getTable = function() {
   	...
   	var table = $("#marketProportionSet_table").DataTable({
        data : $scope.dataset,
...
//       var table = initDataTables("marketProportionSet_table", columns_array, $scope.dataset,120);
   };
   
//   定义保存方法
   $scope.positionSave = function(){
//   	设置持仓占比属性值
   	$scope.entity.tplID = "8";
	...
	layer.load(2, {
   	...
   	$scope.makertservice.save($scope.entity,sessionStorage.listBrokerID,function(result){
   		if(result){
   			layer.alert(result.errMessage);
   			var mytable = document.getElementById("innerMarketTable");
            var outerContainer = document.getElementById("outMarketDiv");
            outerContainer.removeChild(mytable);
            var newContainer = document.createElement("div");
            newContainer.id = "innerMarketTable";
            newContainer.innerHTML = "<table id='marketProportionSet_table' class='cell-border stripe' cellspacing='0' width='100%'></table>";
            outerContainer.appendChild(newContainer);
   			//destroyDatatable("marketProportionSet_table");
   ...
   $('#marketProportionSet_table').on( 'page.dt', function () {
   	setTimeout(function(){
   		var myfilter = document.getElementsByClassName("backFilter");
       	hideAfterFilter($rootScope.filterEntity,myfilter);
   	})
   ...
   $("body").delegate('#marketProportionSet_table_length div').on('change', 'select[name="marketProportionSet_table_length"]', function () {
   	setTimeout(function(){
   		var myfilter = document.getElementsByClassName("backFilter");
       	hideAfterFilter($rootScope.filterEntity,myfilter);
   	})
  ...
  /* $(document).ready(function(){
	   ...
	   setTimeout(function(){
		   $scope.markerProportionSetAddShow = true;
		   $scope.$apply();
	   },"100")
   });*/
...
//   删除
   ...
   localStorage.marketlastname="aa";
   $("body").delegate("#marketProportionSet_table td .market_deleteIndex","click",function(event){
	   event.stopPropagation();
	   if(localStorage.marketlastname == "aa"){
...
//   	修改状态 让 radio 无法选择
   ...
   $scope.update = function(){
	   layer.load(2, {
   	   ...
   	   $scope.makertservice.updatePostionOrMarket($scope.entity,sessionStorage.listBrokerID,function(result){
	   		if(result){
	   			layer.alert(result.errMessage);
	   			var mytable = document.getElementById("innerMarketTable");
                var outerContainer = document.getElementById("outMarketDiv");
   ...
   $scope.marketProportionSet_radionFlag = false;
//   修改按钮点击事件
   $("body").delegate("#marketProportionSet_table td .market_updateIndex","click",function(){
	   
	   $scope.marketProportionSetShow = true;
		$scope.marketProportionSetEdit = false;
	   //为选中的行设置选中色
       ...
       $scope.entity = {};
   	   $scope.initRadio = "";
	   $scope.marketProportionSet_radionFlag = true;
	   var table = $("#marketProportionSet_table").DataTable();
...
//				基金
   ...
   $scope.addPostionSet = function(){
	   	...
	   	$scope.marketProportionSet_radionFlag = false;
	   	//初始化数据
	   	$(".marketProportionSet_contractType").attr("checked",false); 
	   	$(".marketProportionSet_contractType").attr("checked","checked"); 
	   	$scope.statistics = [];
	   	$scope.molecule = [];
	   	$scope.denominator = [];
	   	$scope.riskgroup = [];
