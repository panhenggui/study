...
var configArr = window.require('electron').remote.getGlobal('configArr');
const electron = window.require('electron');
const {ipcRenderer} = electron;

//初始化验证码图片
window.onload = function() {
    if(null != configArr && configArr.length > 1){
        for(var i = 0; i<configArr.length; i++){
            if(window.localStorage.getItem("ipadress") && window.localStorage.getItem("ipadress") == configArr[i].domain){
                $('#ipadress').append("<option selected=true value="+configArr[i].domain+">"+configArr[i].text+"</option>");
                global.frameworkConfig = {
                    domain : configArr[i].domain,
                    wsdomain : configArr[i].wsdomain,
                    project : configArr[i].projectName
                };
            }else if(i == 0 && !window.localStorage.getItem("ipadress")){
                $('#ipadress').append("<option selected=true value="+configArr[i].domain+">"+configArr[i].text+"</option>");
                global.frameworkConfig = {
                    domain : configArr[i].domain,
                    wsdomain : configArr[i].wsdomain,
                ...
                $('#ipadress').append("<option  value="+configArr[i].domain+">"+configArr[i].text+"</option>");
            }
        }
    }else if(null != configArr && configArr.length == 1){
        $('#ipadress').append("<option selected=true value="+configArr[0].domain+">"+configArr[0].text+"</option>");
        global.frameworkConfig = {
            domain : configArr[0].domain,
            wsdomain : configArr[0].wsdomain,
            project : configArr[0].projectName
        };
        window.localStorage.setItem("ipadress",global.frameworkConfig.domain);
    }
    validCodeLoad();
    ...
    $('#ipadress').change(function(){
        var ipAdress = $("#ipadress option:selected").val();
        for(var i = 0; i<configArr.length; i++){
            if(ipAdress == configArr[i].domain){
                global.frameworkConfig = {
                    domain : configArr[i].domain,
                    wsdomain : configArr[i].wsdomain,
                    project : configArr[i].projectName
